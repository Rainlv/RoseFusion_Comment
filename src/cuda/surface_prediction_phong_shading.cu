    ...
    namespace internal {
        namespace cuda {
            ...
            float interpolate_trilinearly(const Vec3fda &point, const PtrStepSz<short> &tsdf_volume,
                ...
                return static_cast<float>(tsdf_volume.ptr(
                        (point_in_grid.z()) * volume_size.y + point_in_grid.y())[point_in_grid.x()]) * DIVSHORTMAX *
                       (1 - a) * (1 - b) * (1 - c) +
                       static_cast<float>(tsdf_volume.ptr(
                               (point_in_grid.z() + 1) * volume_size.y + point_in_grid.y())[point_in_grid.x()]) *
                       DIVSHORTMAX * (1 - a) * (1 - b) * c +
                       static_cast<float>(tsdf_volume.ptr(
                               (point_in_grid.z()) * volume_size.y + point_in_grid.y() + 1)[point_in_grid.x()]) *
                       DIVSHORTMAX * (1 - a) * b * (1 - c) +
                       static_cast<float>(tsdf_volume.ptr(
                               (point_in_grid.z() + 1) * volume_size.y + point_in_grid.y() + 1)[point_in_grid.x()]) *
                       DIVSHORTMAX * (1 - a) * b * c +
                       static_cast<float>(tsdf_volume.ptr((point_in_grid.z()) * volume_size.y + point_in_grid.y())[
                               point_in_grid.x() + 1]) * DIVSHORTMAX * a * (1 - b) * (1 - c) +
                       static_cast<float>(tsdf_volume.ptr((point_in_grid.z() + 1) * volume_size.y + point_in_grid.y())[
                               point_in_grid.x() + 1]) * DIVSHORTMAX * a * (1 - b) * c +
                       static_cast<float>(tsdf_volume.ptr((point_in_grid.z()) * volume_size.y + point_in_grid.y() + 1)[
                               point_in_grid.x() + 1]) * DIVSHORTMAX * a * b * (1 - c) +
                       static_cast<float>(tsdf_volume.ptr(
                               (point_in_grid.z() + 1) * volume_size.y + point_in_grid.y() + 1)[point_in_grid.x() +
                                                                                                1]) * DIVSHORTMAX * a *
                       b * c;
            ...
            void raycast_tsdf_kernel(const PtrStepSz<short> tsdf_volume, const PtrStepSz<uchar3> color_volume,
                ...
                float lens = sqrt(
                        pow(eye_pose_direction_x, 2) + pow(eye_pose_direction_y, 2) + pow(eye_pose_direction_z, 2));
                ...
                float light_cos =
                        normal_x * light_direction_x + normal_y * light_direction_y + normal_z * light_direction_z;
                if (light_cos <= 0) {
                    light_cos = -light_cos;
                }
